...
STEM = "gcicy_tn_audited_source_and_math_v1_0_1"
...
audit reports.  It contains no large point arrays or trained checkpoints.
...
Run `python VERIFY_BUNDLE.py` before reviewing.  In the pinned project
environment, `python -m pytest -q` passes 443 tests.
...
def main() -> None:
    ...
    for name in ("MANIFEST.json", "VERIFY_PATCH.py", "BASE_RELEASE.json"):
        path = STAGING / name
        if path.exists():
    ...
    provenance_record = {
        ...
        "large_data_or_checkpoints_included": False,
    }
