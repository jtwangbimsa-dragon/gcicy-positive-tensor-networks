#!/usr/bin/env python3
"""Audit metric volume and line-bundle slopes against exact intersections."""
...
DEFAULT_ARTIFACTS = ",".join(
    [
        ...
        str(ROOT / "outputs" / "gcicy_generic_global_h_metric_k2_rank40_weighted.npz"),
    ]
)
LINE_BUNDLES = ((1, 0, 0), (0, 1, 0), (0, 0, 1))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--artifacts", default=DEFAULT_ARTIFACTS)
    parser.add_argument("--seeds", default="7101,7102,7103,7104,7105,7106,7107,7108")
    parser.add_argument("--points", type=int, default=2048)
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs" / "gcicy_geometric_observables_audit.json",
    )
...
def main() -> None:
        ...
        artifacts.append(
            {
                "path": path,
                "artifact": artifact,
                "label": "k" + "_".join(str(value) for value in degree),
                "degree": degree,
            }
        )
    ...
    for seed in parse_seeds(args.seeds):
        ...
        proposal_density = np.exp(
            np.asarray([generic_proposal_log_density(point) for point in points], dtype=float)
        )
        factors = np.asarray([generic_factor_metrics(point) for point in points], dtype=np.complex128)
        row = {"seed": seed, "points": args.points, "candidates": {}}
        for candidate in candidates:
            if candidate["label"] == "ambient_fs":
                metrics = generic_baseline_metrics(points)
            else:
    ...
    exact_volume = float(normalized_volume_ratio())
    exact_slopes = [float(normalized_slope_ratio(line_bundle)) for line_bundle in LINE_BUNDLES]
    aggregate = {}
    for candidate in candidates:
    ...
    summary = {
        "description": "Metric volume and ambient line-bundle slope audit against exact gCICY intersections.",
        "model_seed": model_seed,
        "seeds": parse_seeds(args.seeds),
        "points_per_seed": args.points,
        "exact": {
            "volume_ratio": exact_volume,
            "volume_ratio_fraction": str(normalized_volume_ratio()),
            "line_bundles": [list(value) for value in LINE_BUNDLES],
            "slope_ratios": exact_slopes,
            "slope_ratio_fractions": [str(normalized_slope_ratio(value)) for value in LINE_BUNDLES],
        },
        "aggregate": aggregate,
        "rows": seed_rows,
    }
